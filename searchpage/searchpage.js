...
function renderHospitals(data, query = "") {
  ...
  data.forEach(item => {
    const card = `
      <div class="border-t border-gray-300 py-6 cursor-pointer" onclick="redirectToDetails('${item._id}','${item.type}')">
...
window.addEventListener("DOMContentLoaded", () => {
  fetchAllHospitals().then(() => {
    // Step 1: Extract query param from URL
    const urlParams = new URLSearchParams(window.location.search);
    const searchQuery = urlParams.get("search");

    // Step 2: If a query exists, perform the search
    if (searchQuery) {
      handleSearch(searchQuery);
      document.getElementById("hospital-list")?.scrollIntoView({ behavior: "smooth" });
    } else {
      renderHospitals(allHospitalsData); // show all by default
    }
  });
...
function redirectToDetails(hospitalId,type) {


  if (hospitalId && type) {
    window.location.href = `./../hospital-detail-page/hospitaldetail.html?id=${hospitalId}&type=${type}`;
  }
}
